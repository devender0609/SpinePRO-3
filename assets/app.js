...
(function () {
  ...
  function engineAPI(engine) {
    ...
    api.answer = (itemId, choiceIndex) => {
      // Support multiple engine APIs:
      // - answer(choiceIndex)
      // - answer(itemId, choiceIndex)
      // - submit(itemId, choiceIndex) or submit(choiceIndex)
      if (typeof engine.answer === "function") {
        try {
          // If the engine declares a single-arg signature, pass only the choice index
          // (extra args can break engines that interpret arg1 as the response).
          if (engine.answer.length <= 1) return engine.answer(choiceIndex);
          return engine.answer(itemId, choiceIndex);
        } catch (e) {
          // Fall back to the other calling convention
          if (engine.answer.length <= 1) return engine.answer(itemId, choiceIndex);
          return engine.answer(choiceIndex);
        }
      }
      if (typeof engine.submit === "function") {
        try {
          if (engine.submit.length <= 1) return engine.submit(choiceIndex);
          return engine.submit(itemId, choiceIndex);
        } catch (e) {
          if (engine.submit.length <= 1) return engine.submit(itemId, choiceIndex);
          return engine.submit(choiceIndex);
        }
      }
      throw new Error("Engine does not expose an answer/submit method.");
    };
