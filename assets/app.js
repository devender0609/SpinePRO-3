/* SpinePRO CAT runtime (no jQuery
    if (!api || typeof api.nextItem !== 'function' || typeof api.answer !== 'function') {
      throw new Error('CAT engine session API missing required methods (nextItem/answer).');
    }
)
...
(function () {
  ...
  // Catch runtime errors and show them in the UI (prevents "blank screen" failures)
  function attachGlobalErrorHandlers() {
    window.addEventListener('error', (e) => {
      try {
        const msg = (e && e.error && e.error.message) ? e.error.message : (e && e.message ? e.message : 'Unknown error');
        renderError(msg);
      } catch (_) {}
    });
    window.addEventListener('unhandledrejection', (e) => {
      try {
        const msg = (e && e.reason && e.reason.message) ? e.reason.message : String(e && e.reason ? e.reason : 'Unhandled rejection');
        renderError(msg);
      } catch (_) {}
    });
  }
  ...
  async function initSurvey() {
    ...
    async function step() {
      ...
      if (typeof item === "string") {
        if (!bank.items || !bank.items[item]) {
          throw new Error(`Item not found in bank: ${item}`);
        }
        item = bank.items[item];
      }
      ...
      renderItem(item, (choiceIndex) => {
        ...
        try { step(); } catch (e) { renderError(e && e.message ? e.message : String(e)); }
      });
    }
